...
### 4.1.3.tmpfs Mounts（内存挂载）

 📖概念比喻

> **就像"电脑的内存条"**：数据存在 RAM 中，速度极快，但断电（容器停止）就消失。
...
graph LR
    ...
    subgraph Container [容器]
        TmpfsMount["/tmp/cache"]
    end
    
    RAM -.->|内存映射| TmpfsMount
    
    style RAM fill:#ffcdd2,stroke:#c62828
    style TmpfsMount fill:#f8bbd0,stroke:#ad1457
```

 适用场景

- 临时缓存（如 Redis 缓存、会话数据）
- 敏感数据（不希望写入磁盘）
- 高性能临时文件处理
- 构建中间产物
...
```bash
# 1️⃣ 启动容器，创建 tmpfs 挂载
docker run -d \
  --name app \
  --mount type=tmpfs,target=/tmp/cache,tmpfs-size=1G,tmpfs-mode=1777 \
  alpine

# 或者简写（仅 Docker CLI 支持，Compose 不支持）
docker run -d --name app --tmpfs /tmp/cache:size=1G,mode=1777 alpine

# 2️⃣ 写入数据
docker exec app sh -c "echo 'temp data' > /tmp/cache/test.txt"

# 3️⃣ 验证数据存在
docker exec app cat /tmp/cache/test.txt
# 输出：temp data
...
```mermaid
flowchart LR
    A["需要持久化数据？"] -->|否| B{"需要高性能临时存储？"}
    A -->|是| C{"谁管理存储路径？"}
    
    B -->|是| D[使用 tmpfs]
    B -->|否| E[使用容器可写层]
    
    C -->|Docker 管理| F["使用 Volumes ✅ 推荐"]
    C -->|我手动管理| G{"使用场景？"}
    
    G -->|"开发/代码热重载"| H["使用 Bind Mounts"]
    G -->|"生产/配置文件"| I["使用 Volumes 或 Bind Mounts"]
...
services:
  db:
    image: mysql:8.0
    volumes:
      - db_/var/lib/mysql                    # Volume：数据库持久化
    environment:
  ...
  wordpress:
    ...
    volumes:
      - wp_/var/www/html                     # Volume：网站文件
      - ./wp-custom.ini:/usr/local/etc/php/conf.d/custom.ini:ro  # Bind Mount：自定义配置
    environment:
  ...
  redis:
    ...
    volumes:
      - redis_/data                          # Volume：Redis 持久化
      - type: tmpfs
        target: /tmp                         # tmpfs：临时缓存
        tmpfs:
